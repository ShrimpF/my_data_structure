...
impl<T: PartialOrd + Clone> Heap<T> {
    pub fn new(a: &mut Vec<T>) -> Heap<T> {
        Heap::build(a);
        Heap { a: a.to_vec() }
    }

    fn build(a: &mut Vec<T>) {
    ...
    fn heapify(a: &mut Vec<T>, i: usize) {
        let left = 2 * i + 1;
        let right = 2 * i + 2;
        let mut smallest = i;
    ...
    macro_rules! test_new {
        ...
        $(
            ...
            fn $name() {
                ...
                assert_eq!(Heap::new(input), expected);
            }
        )*
        }
    }
